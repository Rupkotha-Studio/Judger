...
function processApiResponseData(response) {
    ...
    if (typeof response.error == 'undefined') {
        if (response.checkerError != "") $("#outputResponse").html("<div class='checkerError'><b><i class='fa fa-exclamation-circle'></i> Checker Error:</b><br/>" + response.checkerError + "</div>");
        else {
            if (response.checkerVerdict == 1) $("#outputResponse").html("<div class='checkerValidVerdict'><i class='fa fa-check-circle'></i> Checker Verdict Is Correct</div>");
            else $("#outputResponse").html("<div class='checkerWrongVerdict'><i class='fa fa-times-circle'></i> Checker Verdict Is Wrong</div>");
